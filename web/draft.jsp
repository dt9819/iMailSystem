     ...
     <th width=10%><b><center>TIME<br>&nbsp;<center></b></th>
         ...
         <tbody><form method="post">
          <%
           ...
           ResultSet resultset = statement.executeQuery("Select mails.* from mails,foldermap where foldermap.uid='" + curuser + "' AND foldermap.folderid=2 AND foldermap.mailid=mails.mailid ORDER BY mails.mailid desc LIMIT " + begid + ",10;");
           while (resultset.next()) {
            i++;
          %>
          <tr>
           <td><%= resultset.getString(2)%></td>
           <td><a href="dcompose.jsp?mid=<%= resultset.getString(1)%>&did=1"><%= resultset.getString(3)%></a></td>
          ...
          <%
           ...
           resultset = statement.executeQuery("Select count(*) from mails,foldermap where foldermap.uid='" + curuser + "' AND foldermap.folderid=2 AND foldermap.mailid=mails.mailid;");
          ...
          <br><center><a href="?b=<%= previd%>">« Previous</a><b> Showing <%= begid + 1%>-<%= begid + i%> of <%= maxmails%> </b><a href="?b=<%= nextid%>">Next »</a></center><input type="submit" value="Delete Checked" style="float:right;" formaction="delCheck.jsp?b=<%= begid%>&f=2"/></form>
         <!-- Mail Body Ends ! -->
