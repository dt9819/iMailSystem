 ...
 <div class="viewmailbox">
  <table border="0" cellspacing="3" id="alter" cellpadding="5" style="width:100%">
   <thead>
    ...
    <tr>
     <th width=30%><b><center>--FROM / TO-- </center></b></th>
     ...
     <th width=10%><b><center>TIME<br>&nbsp;<center></b></th>
         ...
         <tbody><form method="post">
          <%
           ...
           ResultSet resultset = statement.executeQuery("Select mails.* from mails,foldermap where foldermap.uid='" + curuser + "' AND foldermap.folderid=4 AND foldermap.mailid=mails.mailid ORDER BY mails.mailid desc LIMIT " + begid + ",10;");
           while (resultset.next()) {
            i++;
          %>
          <tr>
           <td><%= resultset.getString(2)%> <b>/</b> <%= resultset.getString(5)%></td>
          ...
          <%
           ...
           resultset = statement.executeQuery("Select count(*) from mails,foldermap where foldermap.uid='" + curuser + "' AND foldermap.folderid=4 AND foldermap.mailid=mails.mailid;");
          ...
          <br><center><a href="?b=<%= previd%>">« Previous</a><b> Showing <%= begid + 1%>-<%= begid + i%> of <%= maxmails%> </b><a href="?b=<%= nextid%>">Next »</a></center><input type="submit" value="Delete Checked" style="float:right;" formaction="delCheck.jsp?b=<%= begid%>&f=4"/></form>
         <!-- Mail Body Ends ! -->
